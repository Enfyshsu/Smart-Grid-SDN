# version 01251
# reference: https://wildanmsyah.wordpress.com/2018/01/13/multipath-routing-with-load-balancing-using-ryu-openflow-controller/
...
arp_pkt_cnt = defaultdict(int)
def get_path_num():
    pmus = files.read_file(os.path.join(data_folder, 'data_30.json'))
    for pmu in pmus:
        MAX_PATHS[pmu['ip']] = pmu['path_num']
    return
...
def get_optimal_paths(src, dst, path_num):
    '''
    Get the n-most optimal paths according to MAX_PATHS
    '''
    paths = get_paths(src, dst)
    paths_count = len(paths) if len(paths) < path_num else path_num
...
class ProjectController(app_manager.RyuApp):
    ...
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst, mac_src, mac_dst):
        ...
        path_num = MAX_PATHS[ip_src] if ip_src in MAX_PATHS.keys() else 1
        if src is dst: # if destination is in the same switch
            paths = [[src]]
        else:
            paths = get_optimal_paths(src, dst, path_num)
        pw = []
        for path in paths:
        ...
        for node in switches_in_paths:
            ...
            for in_port in ports:
                ...
                match_ip_mac = ofp_parser.OFPMatch(
                    in_port=in_port,
                    eth_type=0x0800,
                    # ipv4_src=ip_src, 
                    eth_dst=mac_dst
                ...
                # if len(out_ports) > 1:
                if len(set([x[0] for x in out_ports])) > 1:
                    group_id = None
                    group_new = False
                ...
                elif len(set([x[0] for x in out_ports])) == 1:
                    # print 'Match for %s from %s to %s out_ports %d' % (node, src, dst, out_ports[0])
                    actions = [ofp_parser.OFPActionOutput(out_ports[0][0])]

                    self.add_flow(dp, 32768, match_ip, actions)
                    self.add_flow(dp, 1, match_arp, actions)
                    self.add_flow(dp, 10, match_ip_mac, actions)
    ...
    def _packet_in_handler(self, ev):
        ...
        print(pkt)
        #if dst in mymac.keys():
        #    print(dpid, self.mac_to_port[dpid][dst])
        if dst == mac.BROADCAST_STR:
            ...
            #print('--------------')
            #print(arp_pkt_cnt)
            if arp_pkt_cnt[str(arp_pkt.dst_ip) + str(dpid) + str(src) + str(in_port)] > 2:
                return
            else:
                arp_pkt_cnt[str(arp_pkt.dst_ip) + str(dpid) + str(src) + str(in_port)] += 1
        # print pkt
